...
-spec set_event([binary()], binary(), integer(), binary(), boolean(), any()) -> {new | existing, spread_event:event(), [any()]}.
set_event(TopicName, From, Date, DataAsBinary, IsDataFinal, SpecificPropagateRules) ->
    ...
    {NewOrExisting, Event} = spread_event:new(TopicName, From, Date, DataAsBinary, IsDataFinal, SpecificPropagateRules),
